...
from util.type_check import is_variable_formatted, is_constant_formatted
...
def sample_tree_raw(
    ...
    if feature_priors is not None:
        for key in feature_priors:
            if not (is_variable_formatted(key) or is_constant_formatted(key)):
                raise Exception("Use standard formats for feature priors: x_{int} for variables, "
    ...
    if not equation_tree.check_possible(_feature_priors,
                                        _function_priors,
                                        _operator_priors,
                                        _structure_priors):
        return None
...
# test_variable = lambda x: 'x_' in x
# test_constant = lambda x: 'c_' in x
# test_function = lambda x: x in DEFAULT_FUNCTION_SPACE
# test_operator = lambda x: x in DEFAULT_OPERATOR_SPACE
#
# equation_tree = EquationTree.from_prefix(
#     prefix_notation=['*', '-', 'x_2', 'c_1', '+', 'x_2', 'x_1'],
#     variable_test=test_variable,
#     constant_test=test_constant,
#     function_test=test_function,
#     operator_test=test_operator
# )
#
# equation_tree.simplify(test_function, test_operator)


for _ in range(1_000_000):
    sample_tree_iter(random.randint(3, 10), random.randint(1, 3), random.randint(1, 3))
    if not _ % 100:
        print(_)
#
# def sample_trees(
